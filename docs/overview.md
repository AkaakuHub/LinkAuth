...
- `workers/app`:利用アプリのサンプルです。認証の中身は持たず、`link-auth`のapp認証ラッパーを呼びます。
...
- `startAppLogin`:ログイン開始
- `completeAppLogin`:callback処理
- `getAppUser`:現在ユーザー取得と認証検証
- `clearAppSession`:ログアウト
